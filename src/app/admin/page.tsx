"use client";
 ...
 function StatusToggle({
   ...
   return (
     <button
       ...
       aria-label={checked ? "Marcar como confirmado" : "Marcar como concluído"}
       disabled={disabled}
 ...
 const TAB_TITLES: Record<Tab, { title: string; subtitle: string }> = {
   ...
   lembretes: {
     title: "Lembretes",
     subtitle: "Planos próximos do vencimento",
   },
   clientes: {
     title: "Clientes",
   ...
   avaliacoes: {
     title: "Avaliações",
     subtitle: "Aprove ou negue as avaliações dos clientes",
   },
 };
 ...
 export default function AdminPage() {
   ...
   async function handleStatusChange(
     ...
     setUpdatingId(id);
     try {
       const previous = appointments.find((apt) => apt.id === id);
       ...
       if (
         ...
         void fetch("/api/email/feedback-request", {
           method: "POST",
           headers: { "Content-Type": "application/json" },
           body: JSON.stringify({
             clientName: previous.client_name,
             clientEmail: previous.client_email,
             serviceName: previous.services?.name,
             dateBR: formatDateBR(previous.appointment_date),
           }),
         }).catch(() => {});
       }
   ...
   async function handleDenyReview(id: string) {
     try {
       ...
       console.error("Erro ao negar avaliação:", err);
     }
   }
         ...
         <div className="bg-brand-dark border border-white/5 rounded-2xl p-8 max-w-md w-full text-center">
           ...
           <p className="text-gray-400 text-sm mb-6">
             {user
               ...
               : "Você precisa entrar com uma conta autorizada para acessar o painel."}
           </p>
           <Link
             href="/"
           ...
           <div key={tab} className="animate-fade-in-up">
               ...
               <>
                 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                   ...
                   <div className="bg-brand-dark rounded-2xl p-6 border border-white/5">
                     ...
                     <p className="text-gray-500 text-sm mt-1">Concluídos Hoje</p>
                   </div>
               ...
               <div className="bg-brand-dark rounded-2xl border border-white/5">
                 <div className="p-4 md:p-6 border-b border-white/5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                   <div>
                     ...
                     <p className="text-sm text-gray-500 mt-1">
                       Planos que vencem nos próximos 7 dias
                     </p>
                   </div>
                   <button
                           ...
                           <table className="w-full">
                             <thead>
                               <tr className="border-b border-white/5">
                                 ...
                                 <th className="text-left px-4 md:px-6 py-3 text-sm font-medium text-gray-500">Último acesso</th>
                               </tr>
                             </thead>
                             <tbody>
                               {users.map((u) => (
             ...
             {tab === "avaliacoes" && (
               <div className="bg-brand-dark rounded-2xl border border-white/5">
                 <div className="p-4 md:p-6 border-b border-white/5 flex items-center justify-between gap-4">
                   <div>
                     <h3 className="text-xl font-bold text-white">Avaliações dos Clientes</h3>
                     <p className="text-sm text-gray-500 mt-1">
                       {allReviews.filter((r) => !r.approved).length} pendente(s) de aprovação
                     </p>
                   </div>
                 </div>
                 <div>
       ...
       {previewPlan && (
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={() => setPreviewPlan(null)}>
           <div className="bg-brand-dark rounded-2xl p-6 md:p-8 border border-[var(--border-main)] max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
             <div className="flex items-center justify-between mb-6">
               <h3 className="text-xl font-bold text-white">Prévia do E-mail</h3>
             ...
             <div className="rounded-xl overflow-hidden border border-white/10 mb-6">
               ...
               <div style={{background:"#111111",padding:"24px"}}>
                 ...
                 <p style={{color:"#D1D5DB",fontSize:"13px",lineHeight:"20px",textAlign:"center",margin:"0 0 20px"}}>
                   Olá <strong style={{color:"#ffffff"}}>{previewPlan.client_name}</strong>, seu plano
                 ...
                 <div style={{background:"#0a0a0a",borderRadius:"10px",border:"1px solid rgba(255,255,255,0.05)",padding:"16px",marginBottom:"20px"}}>
                   <table style={{width:"100%"}}>
                     <tbody>
                       ...
                       <tr><td colSpan={2}><hr style={{border:"none",borderTop:"1px solid rgba(255,255,255,0.05)",margin:0}}/></td></tr>
                       <tr><td style={{padding:"6px 0",color:"#6B7280",fontSize:"12px"}}>Data de expiração</td><td style={{padding:"6px 0",color:"#ffffff",fontSize:"13px",fontWeight:600,textAlign:"right"}}>{formatDateBR(previewPlan.end_date)}</td></tr>
                     </tbody>
                   </table>
                 </div>
               </div>
       ...
       {previewReminder && (
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={() => setPreviewReminder(null)}>
           <div className="bg-brand-dark rounded-2xl p-6 md:p-8 border border-white/10 max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
             <div className="flex items-center justify-between mb-6">
               <h3 className="text-xl font-bold text-white">Prévia do Lembrete</h3>
             ...
             <div className="rounded-xl overflow-hidden border border-white/10 mb-6">
               ...
               <div style={{background:"#111111",padding:"24px"}}>
                 ...
                 <p style={{color:"#D1D5DB",fontSize:"13px",lineHeight:"20px",textAlign:"center",margin:"0 0 20px"}}>
                   Olá <strong style={{color:"#ffffff"}}>{previewReminder.client_name}</strong>, seu plano
